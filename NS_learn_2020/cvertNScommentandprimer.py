...
"""установить выборку рандома, чтоб вносить изменения и видеть разницу"""
random.seed(0) #установить выборку рандома, чтоб вносить изменения и видеть разницу
np.random.seed(0) #установить выборку рандома, чтоб вносить изменения и видеть разницу
torch.manual_seed(0) #установить выборку рандома, чтоб вносить изменения и видеть разницу
torch.cuda.manual_seed(0) #установить выборку рандома, чтоб вносить изменения и видеть разницу
torch.backends.cudnn.deterministic=True #при библиотеки cudnn, если хотим детерминистичнось
...
plt.imshow(X_train[0, :, :]) #выведем нулевую картинку из тензора
plt.show() #показать картинку
print(y_train[0]) #выведем цифру, которая соответствует этой картинке
...
class LeNet5(torch.nn.Module):
    def __init__(self,activation='tanh',pooling='avg',conv_size=5, use_batch_norm=False): #задали параметры по умолчанию
        ...
        self.fc3 = torch.nn.Linear(84, 10) #прогоняем всё через сеть линейных нейронов; выходов 10, тк всего 10 классов чисел
...
def train(net, X_train, y_train, X_test, y_test):
    ...
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu') #если есть видеокарта, то будем ее применять
    ...
    for epoch in range(30): #каждую эпоху мы будем перемешивать датасет
        ...
        for start_index in range(0, len(X_train), batch_size): #будем вырезать участочки длиной batch_size
            ...
            y_batch = y_batch.type(torch.LongTensor) #из-за разницы типов преобразум в ЛонгТензор
            preds = net.forward(X_batch) #предсказываем y_pred с помощью нейросети на основе данных x_train
        ...
        accuracy = (test_preds.argmax(dim=1) == y_test).float().mean().data.cpu() #сравниваем с тем номером класса, который находится в y_test
