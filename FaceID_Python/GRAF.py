...
y_train=-torch.atan(x_train)/5 #создаём y_train с синусоидальной зависимостью от x_train
...
y_validation=-torch.atan(x_validation.data)/5 #определим синус этих точек
...
for epoch_index in range(500): #каждую эпоху мы будем перемешивать датасет
...
plt.hlines(0,-10 ,10)
plt.vlines(0,-0.35, 0.35)

for i in range(100):
    plt.vlines(x_validation[0.125*i*i],-0.35,y_validation[i])
